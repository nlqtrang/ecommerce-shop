    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("user") != null) {
            ...
            for (String checkbox : checkboxes) {
                listProductIds += checkbox + ",";
            }
            listProductIds = listProductIds.substring(0, listProductIds.length() - 1);

            //        response.getWriter().print(listProductIds);
            List<ProductsInCartDTO> listProducts = new CartsDAO().listProductsCheckOut(userId, listProductIds);
            request.setAttribute("listProducts", listProducts);
            request.setAttribute("listProductIds", listProductIds);
